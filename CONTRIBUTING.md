...
**Option 1: Multiple binaries in one crate (recommended for related experiments)**
```
cd labs
cargo new ownership-moves --bin
```

**`labs/ownership-moves/Cargo.toml`:**
```
[[bin]]
name = "basics"
path = "src/basics/main.rs"

[[bin]]
name = "moves"
path = "src/moves/main.rs"
```

**Run:** `cargo run -p ownership-moves --bin basics`

**Option 2: Single binary per concept**
```
cd labs
cargo new borrowing --bin
```

1. Add the new crate to the workspace by listing it in the root `Cargo.toml` under `members`.

2. Write the code:
   - For simple labs: minimal `src/main.rs`, small number of lines, easy to run or break.
...
3. (Recommended) Add a `notes.md` in the lab directory, covering:
...
4. (If adding a new lab) Optional: add a line in the main `README.md` table referencing the new lab.
...
- If a lab grows in scope, consider splitting into multiple binaries or labs each focusing on a single concept
